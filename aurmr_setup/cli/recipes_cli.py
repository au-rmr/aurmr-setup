...
def get_system_scripts() -> List["Recipe"]:
    scripts = [
        os.path.splitext(s)[0] for s in files(system_scripts) if s.endswith(".sh")
    ]
...
def system_prepare(software: str):
    ...
    if not questionary.confirm(
        f"Do you really want to run these scripts: {software}", default=False
    ).ask():
        sys.exit(1)
...
def get_user_scripts() -> List[str]:
    scripts = [os.path.splitext(s)[0] for s in files(user_scripts) if s.endswith(".sh")]
    return sorted(scripts)
