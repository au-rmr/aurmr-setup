...
class Recipe:
    ...
    def list_all(cls, category) -> List["Recipe"]:
        scripts = [os.path.splitext(s)[0] for s in files(category) if s.endswith(".sh")]
...
class UserRecipe(Recipe):
    ...
    def all_user_scripts_str(cls):
        return [str(r) for r in UserRecipe.all_user_scripts()]
...
class SystemRecipe(Recipe):
    ...
    def all_system_scripts_str(cls):
        return [str(r) for r in SystemRecipe.all_system_scripts()]
